...
// Each composition's own name is that fallback: the imperfect machine is sold as
// imperfect computers, and a dialog greeting its owner as Guey is naming the
// foundation to somebody who never bought the foundation.
export function brandFor(product) {
  return product === 'imperfect' ? 'imperfect computers' : 'Guey';
}
export function resolveBrand(options = {}) {
  ...
  if (!name || !/^[\w .'-]{1,32}$/.test(name)) return brandFor(options.product);
  return name;
}
